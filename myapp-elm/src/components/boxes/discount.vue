<template>
  <div>
    <div class="ttx_discount">
      <!--顶部-->
      <header class="ttx_head_top">
        <section class="ttx_head_goback">
          <router-link to="/balance">
            <svg class="ttx_head_sev">
              <polyline points="12,18 4,9 12,0" style="fill: none; stroke: rgb(255, 255, 255); stroke-width: 2px;">

              </polyline>
            </svg>
          </router-link>
        </section>
        <section class="ttx_title_head_ellipsis">
          <span class="ttx_title_text">我的优惠</span>
        </section>
      </header>

      <div>
        <div class="ttx_category_title">
            <div id="ttx-id1" :class="{ttx_choosed:ttxshow3}" @click="ttxOn1">红包</div>
            <div id="ttx-id2" :class="{ttx_choosed:ttxshow4}" @click="ttxOn2">商家代金券</div>
        </div>
      </div>
      <!--三个红包的顶部-->
      <div v-show="ttxshow1">
        <div class="ttx_hongbao_title">
          <div class="ttx_expire">有<span id="s2">3</span>个红包即将到期</div>
          <div class="ttx_explain">
            <img
              src="data:image/png;base64,"
              alt="">
            <router-link to="/hbsm">
            <span id="s1">红包说明</span>
            </router-link>
          </div>

        </div>
        <ul style="background: rgb(245,245,245)" class="hongbao_list_ul">
          <li class="hongbao_list_li">
            <div class="list_item">
              <div class="ttx_list_item_left">
                <span style="color: red;">¥</span>
                <span style="font-size: 1.5rem;color: red">1</span>
                <span style="color: red">.</span>
                <span style="color: red">0</span>
                <p style="color: #999;font-size: .4rem">满 20 元可用</p>
              </div>
              <div class="ttx_list_item_right">
                <h4 class="h1">分享红包</h4>
                <p class="p1">2019-08-24到期</p>
                <p class="p2">限收货手机号为 </p>
                <p class="p3">962325513</p>
              </div>
              <div class="ttx_time_left">
                剩三日
              </div>
            </div>
          </li>
          <!---->
          <li class="hongbao_list_li">
            <div class="list_item">
              <div class="ttx_list_item_left">
                <span style="color: red;">¥</span>
                <span style="font-size: 1.5rem;color: red">2</span>
                <span style="color: red">.</span>
                <span style="color: red">0</span>
                <p style="color: #999;font-size: .4rem">满 40 元可用</p>
              </div>
              <div class="ttx_list_item_right">
                <h4 class="h1">分享红包</h4>
                <p class="p1">2019-08-24到期</p>
                <p class="p2">限收货手机号为 </p>
                <p class="p3">962325513</p>
              </div>
              <div class="ttx_time_left">
                剩三日
              </div>
            </div>
          </li>
          <!---->
          <li class="hongbao_list_li">
            <div class="list_item">
              <div class="ttx_list_item_left">
                <span style="color: red;">¥</span>
                <span style="font-size: 1.5rem;color: red">4</span>
                <span style="color: red">.</span>
                <span style="color: red">5</span>
                <p style="color: #999;font-size: .4rem">满 30 元可用</p>
              </div>
              <div class="ttx_list_item_right">
                <h4 class="h1">分享红包</h4>
                <p class="p1">2019-08-24到期</p>
                <p class="p2">限收货手机号为 </p>
                <p class="p3">962325513</p>
              </div>
              <div class="ttx_time_left">
                剩三日
              </div>
            </div>
            <footer class="list_item_footer">
              <p id="p5">限品类：快餐便当、特色菜系、小吃夜宵、甜品饮品、异国料理</p>
            </footer>
          </li>
        </ul>

        <!--底部-->
        <div class="history_hongbao">
          <span class="check_history">查看历史红包 ></span>
          <footer class="hongbao_footer">
            <router-link to="/exchange">
            <div class="hongbao_style">兑换红包</div>
            </router-link>

            <div class="hongbao_style">推荐有奖</div>

          </footer>
        </div>
      </div>
      <!--商家代金券-->
      <div style="background: rgb(245,245,245);height: 480px" v-show="ttxshow2">
        <div class="voucher_container">
          <div class="hongbao_description voucher_header">
            <img  id="imgs1"  src="data:image/png;base64," alt="">
            <span id="span1">商家代金券说明</span>
          </div>
          <div class="unable_use">
            <img style="width: 6rem;height: 3.4rem" src="data:image/png;base64," alt="">
            <p id="ttx_p10">无法使用代金券</p>
            <p id="ttx_p11">非客户端或客户端版本过低</p>
            <p id="ttx_p12">下载或升级客户端</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
  export default {
    name: "discount",
    data() {
      return {
        ttxshow1: true,
        ttxshow2:false,
        ttxshow3:true,
        ttxshow4:false,
      }
    },
    methods: {
      ttxOn1(ev) {
        if (this.ttxshow1 == true||this.ttxshow2==true){
          this.ttxshow1 = true;
          this.ttxshow2=false;
          this.ttxshow4 = false;
          this.ttxshow3 = true;
        }else {
          this.ttxshow1 = true;
          this.ttxshow2=false;
          this.ttxshow4 = false;
          this.ttxshow3 = true;
        }
      },
      ttxOn2(){
        if (this.ttxshow2 == true||this.ttxshow1==true){
          this.ttxshow2 = true;
          this.ttxshow1=false;
          this.ttxshow3 = false;
          this.ttxshow4 = true;
        }else {
          this.ttxshow2 = true;
          this.ttxshow1=false;
          this.ttxshow3 = false;
          this.ttxshow4 = true;
        }
      }
    }
  }
</script>

<style scoped>
  #ttx-id1{
    padding-bottom: 0.4rem;
  }
  #ttx-id2{
    padding-bottom: 0.4rem;
  }
  .ttx_choosed{
    border-bottom: 1px solid #3190e8;
    color: #3190e8;
    padding-bottom: 0.2rem;
  }
  .ttx_head_top {
    background-color: #3190e8;
    position: fixed;
    z-index: 100;
    left: 0;
    top: 0;
    width: 100%;
    height: 1.95rem;
  }

  .ttx_head_goback {
    left: .4rem;
    width: .6rem;
    height: 1rem;
    line-height: 2.2rem;
    margin-left: .4rem;
  }

  .ttx_head_sev {
    width: 0.8rem;
    float: left;
    height: 1rem;
    margin-top: .4rem;
    z-index: 10;
  }

  .ttx_title_head_ellipsis {
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    width: 50%;
    color: #fff;
    text-align: center;
  }

  .ttx_category_title {
    margin-top: 2rem;
    display: flex;
    justify-content: space-around;
    align-items: center;
    height: 2rem;
    background-color: #fff;
  }

  .ttx_category_title span {
    text-align: center;
    font-size: .65rem;
    /*padding-bottom: .2rem;*/
    /*border-bottom: .1rem solid red;*/
  }

  .ttx_hongbao_title {
    display: flex;
    justify-content: space-around;
    font-size: .6rem;
    height: 1.5rem;
    align-items: center;
    background: rgb(245, 245, 245);

  }

  .ttx_explain img {
    width: .6rem;
  }

  #s1 {
    color: #3190e8;
  }

  #s2 {
    color: red;
  }

  .hongbao_list_ul {
    padding: .4rem;

  }

  .hongbao_list_li {
    background: #fff url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAIBAMAAAALs8LeAAAABGdBT…RBVAjXY5gJBwzEMScHNXpCmQIMDMwQ5jQGIKgEMyeBmJIAtqcnWdDMwKsAAAAASUVORK5CYII=) repeat-x;
    background-size: .5rem .2rem;
    margin-bottom: .6rem;
    border-radius: .25rem;
    margin-bottom: .4rem;

  }

  .list_item {
    display: flex;
    justify-content: space-between;
    padding: 1rem .5rem .8rem;
    line-height: 1.5;
  }

  .ttx_list_item_left {
    border-right: .025rem dotted #ccc;
  }

  .ttx_time_left {
    margin-left: 3rem;
    font-size: .45rem;
    color: #ff5340;
  }

  .h1 {
    font-size: .7rem;
    color: #666;
    margin-left: .7rem;
  }

  .p1 {
    /*list-style-type: disc;*/
    margin-left: .7rem;
    font-size: .4rem;
    color: #999;
  }

  .p2 {
    /*list-style-type: disc;*/
    margin-left: .7rem;
    font-size: .4rem;
    color: #999;
  }

  .p3 {
    /*list-style-type: disc;*/
    margin-left: .7rem;
    font-size: .4rem;
    color: #999;
  }

  .list_item_footer {
    background-color: #f9f9f9;
    padding: .4rem;
    border-bottom-left-radius: .25rem;
    border-bottom-right-radius: .25rem;
  }

  #p5 {
    list-style-type: disc;
    font-size: .4rem;
    color: #999;
    margin-left: .4rem;
  }

  .hongbao_footer {
    position: fixed;
    bottom: 0;
    width: 100%;
    background-color: #fff;
    display: -ms-flexbox;
    display: flex;
  }

  .hongbao_style {
    flex: 1;
    line-height: 2rem;
    text-align: center;
    font-size: .7rem;
    color: #555;
  }

  .history_hongbao {
    padding: .5rem 0 2.8rem;
    display: -ms-flexbox;
    display: flex;
    -ms-flex-align: center;
    align-items: center;
    -ms-flex-pack: center;
    justify-content: center;
    background: rgb(245, 245, 245);
  }

  .check_history {
    font-size: .5rem;
    color: #999;
    margin-right: .2rem;
  }
  #imgs1{
    width: .6rem;
    height: .6rem;
    margin-right: .2rem;
  }
  #span1{
    font-size: .4rem;
    color: #3190e8;

  }
  .hongbao_description{
    text-align: right;
    padding: .5rem;
  }
  .unable_use{
    text-align: center;
    margin-top: 4rem;

  }

  #ttx_p10{
    font-size: .7rem;
    color: #666;
    margin-top: .4rem;
  }
  #ttx_p11{
    font-size: .5rem;
    color: #999;
    margin-top: .3rem;
    margin-bottom: .3rem;
    margin-left: -.5rem;
  }
  #ttx_p12{
    width: 5.5rem;
    background-color: #56d176;
    font-size: .65rem;
    color: #fff;
    padding: .4rem;
    border-radius: .15rem;
    margin: 0 auto;
    /*margin-top: 3rem;*/
  }
</style>
